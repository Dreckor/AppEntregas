...
export const updateConfig = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      for (const category of req.body.productCategories) {
        if (!category._id) {
          ...
          // Si ya tiene un ID, buscarlo y agregarlo solo si no está ya en la configuración
          let existingProductCategory = await ProductCategory.findById(
            category._id
          );
          ...
          if (!exists) {
            ...
            console.log("actualizando la conf ");
            let index = config.productCategories.findIndex(
              (c) => c._id.toString() == category._id
            );
            console.log(index);
            if (index != null) {
    ...
    // Verificación para paymentMethods
    if (req.body.paymentMethods && Array.isArray(req.body.paymentMethods)) {
      for (const method of req.body.paymentMethods) {
        if (!method._id) {
          ...
          const exists = config.paymentMethods.some(
            (p) => p._id.toString() === method._id
          );
          ...
          if (!exists) {
            ...
            console.log("actualizando la conf ");
            let index = config.paymentMethods.findIndex(
              (c) => c._id.toString() == method._id
            );

            if (index != null) {
              config.paymentMethods[index] = category;
              console.log("uptated");
            }
          }
        }
    ...
    if (req.body.states && Array.isArray(req.body.states)) {
      for (const state of req.body.states) {
        if (!state._id) {
          ...
          const exists = config.states.some(
            (p) => p._id.toString() === state._id
          );
...
export const deleteConfigOption = async (req, res) => {
  ...
  if (req.user.role !== "admin") {
    return res
      .status(403)
      .json({
        message: "Solo un usuario administrador puede eliminar la config",
      });
  }
  ...
  try {
    ...
    if (optionType === "deliveryPoints") {
      ...
      config.deliveryPoints = config.deliveryPoints.filter(
        (point) => point._id.toString() !== optionValue
      );
      ...
      // Filtrar y eliminar de la configuración
      config.departurePoints = config.departurePoints.filter(
        (point) => point._id.toString() !== optionValue
      );
      ...
      // Filtrar y eliminar de la configuración
      config.productCategories = config.productCategories.filter(
        (category) => category._id.toString() !== optionValue
      );
      ...
      // Filtrar y eliminar de la configuración
      config.states = config.states.filter(
        (category) => category._id.toString() !== optionValue
      );
